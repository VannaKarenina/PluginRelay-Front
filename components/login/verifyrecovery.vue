...
async function verifyCode() {
  ...
  const sign = await $fetch(`${config.public.baseUrl}/v1/account/recoveryConfirm`, {
    method: 'POST',
    body: {
      loginOrEmail: props.loginOrEmail,
  ...
  if (sign.code == 805) {
    err.value.error = sign.message;
    err.value.close();
  }

  if (sign.code == 802) {
    emits('signature', sign.signature)
  }
